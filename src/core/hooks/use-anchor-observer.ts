      ...
      (enteries) => {
        for (const entry of enteries) {
          if (
            ...
            !visibleHeadings.includes(entry.target.id)
          ) {
            visibleHeadings = [...visibleHeadings, entry.target.id];
          } else if (
            !entry.isIntersecting &&
            visibleHeadings.includes(entry.target.id)
          ) {
        ...
        if (visibleHeadings.length > 0) {
          setActiveAnchor(visibleHeadings);
        }
      },
      {
