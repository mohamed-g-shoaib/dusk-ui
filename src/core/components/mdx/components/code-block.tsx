...
export function CodeBlock({ children, className, ...props }: CodeBlockProps) {
  ...
  return (
    <div className="DuskCodeBlockWrapper [&+.DuskCodeBlockWrapper]:mt-6 mt-4 [.DuskInstallationCommands+&]:mt-8 rounded-2xl bg-code border border-border relative bg-clip-padding before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-2xl)-1px)] before:shadow-[0_1px_2px_1px_--theme(--color-black/4%)] after:pointer-events-none after:absolute after:-inset-[5px] after:-z-1 after:rounded-[calc(var(--radius-2xl)+4px)] after:border after:border-border/50 after:bg-clip-padding ">
      {filePaths.length > 0 ? (
